...
第二步已加入二元 `+`、`-` 和括号。当前支持一个无参数 `int main(void)`
（也接受 `main()`），函数体只有 `return 表达式;`。
表达式中的整数常量为 0～32767，接受十进制、八进制和十六进制，以及空白和 C 注释。
加减同级、从左向右结合；括号可以嵌套；运算结果可为负数。

不支持一元正负号（`-1` 可写成 `0 - 1`）、常量后缀、乘除、变量、参数、其他函数、
预处理指令或头文件。这些输入返回非零状态并报错，不会默默跳过。
所有中间结果都必须落在有符号 16 位范围 −32768～32767；例如 `(32767 + 1) - 1`
虽然最终数值在范围内，也会在溢出的那一步报错，不把硬件回绕当作有符号 C 语义。
当前限定括号最多嵌套 64 层、二元运算最多 256 次，以限制宿主解析递归和目标程序资源使用。
...
python compiler/test_stage2.py
```
...
`test_stage2.py` 会先运行完整 `test_stage1.py` 回归（也可单独运行），重建宿主编译器并验证：

- 手写参考汇编（独立检查启动及调用约定）；
- 编译 example，经 CLI 生成的机器码必须与参考完全一致；
- 13 组常量/记法组合，检查 R0、SP、PC、返回地址小端存放，且没有意外内存写；
- 20 组不支持或错误输入，包括数值溢出、变量、乘法、参数、其他函数和宏；
- 失败编译不覆盖上一次成功产物。

然后运行 52 组表达式仿真，包括左右结合差异、嵌套括号、负数、中间边界值、
不同进制与注释、64 层括号、256 次连续运算和固定种子的 24 组随机表达式。
测试使用独立的 Python AST 计算期望结果、PUSH 次数与最大临时栈深度；
testbench 在 RTL 运行中逐次检查 LIFO 访存、返回地址未被覆盖，以及最终 R0/SP/PC。
另有 42 组错误表达式，覆盖缺操作数、缺括号、未开放语法、溢出及资源上限。

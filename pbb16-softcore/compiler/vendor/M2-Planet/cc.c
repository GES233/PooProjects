...
int main(int argc, char** argv)
{
	...
	while(i <= argc)
	{
		...
		else if(match(argv[i], "-A") || match(argv[i], "--architecture"))
		{
			...
			if(match("pbb16", arch)) {
				Architecture = PBB16;
				init_macro_env("__pbb16__", "1", "--architecture", env);
				env = env + 1;
			}
			else if(match("knight-native", arch)) {
	...
	{
		if(Architecture == PBB16) pbb16_validate_program();
		fputs("Either no input files were given or they were empty\n", stderr);
		exit(EXIT_FAILURE);
	}
	global_token = reverse_list(global_token);
	/* Reject unsupported input before preprocessing or partial code generation. */
	if(Architecture == PBB16) pbb16_validate_program();
	...
	if(Architecture == PBB16)
	{
		fputs("; M2-Planet 1.13.1 / PBB16 stage 2\n", destination_file);

...
| `cc_emit.c` | PBB16 标签格式、立即数发射接口、表达式 `PUSH R0` / `POP R1` |
| `cc_core.c` | PBB16 函数注释使用 `;`；加减运算使用 `ADD` / `SUB` / `MOV` |

本地后端主体放在 `../pbb16_target.c`（GPL-3.0-or-later）。
能力检查已扩展到二元加减和括号，并检查 16 位有符号中间结果及表达式规模；
括号解析和递归求值发射仍使用上游前端。
其他架构分支保留；本仓库只验证 PBB16 阶段 1～2，没有声称完成上游全架构回归。
